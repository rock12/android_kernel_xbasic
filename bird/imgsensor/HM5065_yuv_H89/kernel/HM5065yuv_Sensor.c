...
////burst mode 車谷車迆那?16bit addr o赤 8bit data㏒??谷辰?2???豕???
...
void HM5065InitialSetting(void)
{
	...
	HM5065_write_cmos_sensor(0x00B3,0x30);//80->c0 by 0515
	...
	HM5065_write_cmos_sensor(0x0130,0x00);//00->fe->f9	->00	
	...
	HM5065_write_cmos_sensor(0x02a2,0x3d);	//20140512
	...
	HM5065_write_cmos_sensor(0x02a8,0x3d);// 3e->3d changed 0425
	HM5065_write_cmos_sensor(0x02a9,0x50);//00->50 changed 0425
	...
	HM5065_write_cmos_sensor(0x0081,0x6c); //6E
	...
	HM5065_write_cmos_sensor(0x0080,0x58);
	HM5065_write_cmos_sensor(0x0082,0x6A);	//20140512	Zeroy
#if 0
...
/*************************************************************************
//∩?o‘那y㊣?芍??∩?“辰?,???迄?Y㊣角辰??“辰?那y?Y㊣?芍?.
//prview 1280*960 
//16米???那y㊣? ; n*16*80/1280
//16米???那y㊣? ; n*16*60/960
//touch_x  ?a??車|preview米?o芍℅?㊣那[0-1280]
//touch_y  ?a??車|preview米?℅Y℅?㊣那[0-960]
    ...
    HM5065SENSORDB("[HM5065]CONTROLFLOW enter HM5065_set_brightness function:\n ");
		...
		switch (para)
		{
			...
			case ISP_BRIGHT_MIDDLE:
				  HM5065_write_cmos_sensor(0x0130,0x00);//00->fe->00	20140512 Zeroy
			default:	
				 break; 
		}
...
void HM5065_set_saturation(UINT16 para)
{
    ...
    switch (para)
    {
        ...
        case ISP_SAT_MIDDLE:
        default:
             HM5065_write_cmos_sensor(0x0081 ,0x6c); //54->60
             break; 
    }	
...
void HM5065_set_iso(UINT16 para)
{
	...
	HM5065_write_cmos_sensor(0x015C, 0x3E);
	HM5065_write_cmos_sensor(0x015D, 0x00);
	HM5065_write_cmos_sensor(0x015E, 0x41);
	HM5065_write_cmos_sensor(0x015F, 0x00);
	HM5065_write_cmos_sensor(0x02C0, 0x00);
	HM5065_write_cmos_sensor(0x02C1, 0x00);
	HM5065_write_cmos_sensor(0x02C2, 0x00);
