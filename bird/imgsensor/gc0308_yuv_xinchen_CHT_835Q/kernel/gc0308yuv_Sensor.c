...
void GC0308_Sensor_Init(void)
{
	...
	GC0308_write_cmos_sensor(0xe8 , 0x03);   //exp level 2  8.33fps
	GC0308_write_cmos_sensor(0xe9 , 0xc0); 
	GC0308_write_cmos_sensor(0xea , 0x06);   //exp level 3  4.00fps
	...
	GC0308_write_cmos_sensor(0x14 , 0x13);//10 ->13
	...
	GC0308_write_cmos_sensor(0x62 , 0x0c);//08
	GC0308_write_cmos_sensor(0x64 , 0x01);//03
	...
	GC0308_write_cmos_sensor(0x77 , 0x65);//36
...
void GC0308_Write_More_Registers(void)
{
	...
	GC0308_write_cmos_sensor(0xb3 , 0x3d);//40
...
BOOL GC0308_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0308_write_cmos_sensor(0xe7  ,0x20); 
			GC0308_write_cmos_sensor(0xe8  ,0x03);   //exp level 2  7.69fps
			GC0308_write_cmos_sensor(0xe9  ,0xc0); 
...
BOOL GC0308_set_param_exposure(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AE_EV_COMP_n03:
			...
			GC0308_write_cmos_sensor(0xd3, 0x44);//48
		break;				
		
		case AE_EV_COMP_00:
			GC0308_write_cmos_sensor(0xb5, 0xfd);//00
			GC0308_write_cmos_sensor(0xd3, 0x48);//50
		break;
