...
static unsigned char rg_ratio = 0;
static unsigned char bg_ratio = 0;
...
{   
    ...
    if (index > 2)
	{
		OV8825DB("OTP input wb group index %d error\n", index);
		return -1;
	}
	...
	if (!flag)
	{
		OV8825DB("wb group %d is empty\n", index);
		return 0;
	}
	else if ((!(flag&0x80)) && (flag&0x7f))
	{
		OV8825DB("wb group %d has valid data\n", index);
		return 2;
	}
	else
	{
		OV8825DB("wb group %d has invalid data\n", index);
		return 1;
	}
}
...
static signed char otp_read_wb_group(signed char index)
{
	...
	if (index == -1)
	{
		...
		if (index > 2)
		{
			OV8825DB("no group has valid data\n");
			return -1;
		}
	}
	else
	{
		if (otp_check_wb_group(index) != 2)
		{
			OV8825DB("read wb from group %d failed\n", index);
			return -1;
		}
	}
...
static bool otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	{
		OV8825DB("golden_rg / golden_bg can not be zero\n");
		return 0;
	}
	...
	OV8825DB("cmp_rg=%f, cmp_bg=%f\n", cmp_rg, cmp_bg);
	OV8825DB("ratio_r=%f, ratio_g=%f, ratio_b=%f\n", ratio_r, ratio_g, ratio_b);
	OV8825DB("gain_r=0x%x, gain_g=0x%x, gain_b=0x%x\n", gain_r, gain_g, gain_b);
	return 1;
}
...
static bool otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	OV8825DB("cmp_rg=%d, cmp_bg=%d\n", cmp_rg, cmp_bg);
	OV8825DB("ratio_r=%d, ratio_g=%d, ratio_b=%d\n", ratio_r, ratio_g, ratio_b);
	OV8825DB("gain_r=0x%x, gain_g=0x%x, gain_b=0x%x\n", gain_r, gain_g, gain_b);
	return 1;
}
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool otp_update_wb_ov8825(unsigned char golden_rg, unsigned char golden_bg) 
{
	OV8825DB("start wb update\n");

	if (otp_read_wb_group(-1) != -1)
	{
		if (otp_apply_wb(golden_rg, golden_bg) == 1)
		{
			OV8825DB("wb update finished\n");
			return 1;
		}
	}

	OV8825DB("wb update failed\n");
	return 0;
}
...
{   
    ...
    if (index > 2)
	{
		OV8825DB("OTP input lenc group index %d error\n", index);
		return -1;
	}
	...
	if (!flag)
	{
		OV8825DB("lenc group %d is empty\n", index);
		return 0;
	}
	else if (flag == 0x40)
	{
		OV8825DB("lenc group %d has valid data\n", index);
		return 2;
	}
	else
	{
		OV8825DB("lenc group %d has invalid data\n", index);
		return 1;
	}
}
...
static signed char otp_read_lenc_group(int index)
{
	...
	if (index == -1)
	{
		...
		for (index=0; index<3; index++)
		{
			...
			{
				OV8825DB("read lenc from group %d\n", index);
				break;
			}
		}
	...
	else
	{
		if (otp_check_lenc_group(index) != 2) 
		{
			OV8825DB("read lenc from group %d failed\n", index);
			return -1;
		}
	}
...
{
	OV8825DB("start lenc update\n");

	if (otp_read_lenc_group(-1) != -1)
	{
		otp_apply_lenc();
		OV8825DB("lenc update finished\n");
		return 1;
	}

	OV8825DB("lenc update failed\n");
	return 0;
}
...
UINT32 OV8825Open(void)
{
	...
	spin_unlock(&ov8825mipiraw_drv_lock);
	OV8825_Sensor_Init();
        otp_update_wb_ov8825(0x4E, 0x4E);
        otp_update_lenc();
