 ...
 * Reference:none
 *
 * Revision 1.2  2013/05/10 03:44:56  huangwenjian
 * Bug[none]
 *
 * Reason:12Y36_t1000  ov8825 lens
 * Author:Huangwenjian
 *
 * Revision 1.1.2.1  2013/04/25 10:01:10  huangwenjian
 * Bug[none]
 *
 * Reason:12Y36 T1000 OTP
 * Author:Huangwenjian
 *
 ...
 * Author:Huangwenjian
 *
 * Revision 1.2  2013/04/17 13:15:19  huangwenjian
 * Bug[none]
 *
 * Reason:12Y36 T1000  for OV5647
 * Author:Huangwenjian
 *
...
{
	OV5647_write_cmos_sensor(OTP_LOAD_ADDR, 0x01);
...
{
	OV5647_write_cmos_sensor(OTP_LOAD_ADDR, 0x00);
}

void otp_read(unsigned short otp_addr, unsigned char* otp_data)
{
	otp_read_enable();
	*otp_data = OV5647_read_cmos_sensor(otp_addr);
	otp_read_disable();
}
...
void otp_clear(void)
{
	...
	for (i=0; i<32; i++) 
	{
		OV5647_write_cmos_sensor(OTP_DATA_ADDR+i, 0x00);
	}
}
...
bool otp_apply_wb(unsigned char golden_rg, unsigned char golden_bg)
{
	...
	if (ratio_r != 1)
	{
		...
		OV5647_write_cmos_sensor(GAIN_RH_ADDR, gain_r >> 8);
		OV5647_write_cmos_sensor(GAIN_RL_ADDR, gain_r & 0x00ff);
	}

	if (ratio_g != 1)
	{
		gain_g = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_g);
		OV5647_write_cmos_sensor(GAIN_GH_ADDR, gain_g >> 8);
		OV5647_write_cmos_sensor(GAIN_GL_ADDR, gain_g & 0x00ff);
	}

	if (ratio_b != 1)
	{
		gain_b = (unsigned short)(GAIN_DEFAULT_VALUE * ratio_b);
		OV5647_write_cmos_sensor(GAIN_BH_ADDR, gain_b >> 8);
		OV5647_write_cmos_sensor(GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
static void OV5647_Sensor_Init(void)
{
	...
	OV5647_write_cmos_sensor(0x4708, 0x01);
