...
u8 SP0A19_Initialize_from_T_Flash()
{
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			SP0A19_Init_Reg[i].init_reg = strtol_1((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			SP0A19_Init_Reg[i].init_val = strtol_1((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			SP0A19_Init_Reg[i].init_val = strtol_1((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
...
void SP0A19NightMode(kal_bool bEnable)
{
    ...
    if (bEnable)//night mode
    {
        ...
        if (SP0A19_MPEG4_encode_mode == KAL_TRUE)
        {
            ...
            {
                //SI13_SP0A19 24M 1\B7\D6频 50Hz 8.0392-8fps AE_Parameters_ maxgain 80
...
//SP0A19 ini
    ...
    //awb 1
    SP0A19_write_cmos_sensor(0xfd,0x01);
    SP0A19_write_cmos_sensor(0x28,0xb8);//0xc5 yc 2014-04-01
    ...
    SP0A19_write_cmos_sensor(0x59,0x18); //raw_dif_thr_lowlight
    //GrGb
    ...
    SP0A19_write_cmos_sensor(0x9d,0x18); //raw_grgb_thr_lowlight
    //Gr\Gb
    SP0A19_write_cmos_sensor(0x81,0xe0); //raw_gflt_fac_outdoor
    SP0A19_write_cmos_sensor(0x82,0xb0); //raw_gflt_fac_normal
    SP0A19_write_cmos_sensor(0x83,0x60); //raw_gflt_fac_dummy
    SP0A19_write_cmos_sensor(0x84,0x20); //raw_gflt_fac_lowlight
    //GrGb
    SP0A19_write_cmos_sensor(0x85,0xe0); //raw_gf_fac_outdoor
    SP0A19_write_cmos_sensor(0x86,0xb0); //raw_gf_fac_normal
    SP0A19_write_cmos_sensor(0x87,0x60); //raw_gf_fac_dummy
...
#if 1//lj_test
    SP0A19_write_cmos_sensor(0xe8,0x28);  //sharp_fac_pos_outdoor  // 0x35
    SP0A19_write_cmos_sensor(0xec,0x2c);  //sharp_fac_neg_outdoor
    SP0A19_write_cmos_sensor(0xe9,0x20);  //sharp_fac_pos_nr	  // 0x35
    SP0A19_write_cmos_sensor(0xed,0x23);  //sharp_fac_neg_nr
    SP0A19_write_cmos_sensor(0xea,0x14);  //sharp_fac_pos_dummy   // 0x30
    SP0A19_write_cmos_sensor(0xef,0x18);  //sharp_fac_neg_dummy   // 0x20
    SP0A19_write_cmos_sensor(0xeb,0x0e);  //sharp_fac_pos_low
    SP0A19_write_cmos_sensor(0xf0,0x12);  //sharp_fac_neg_low
#else
    ...
    SP0A19_write_cmos_sensor(0xd6,0x74); //0x78 	   (0xd7,0x60); //0x78  //lj_test 0x70
    SP0A19_write_cmos_sensor(0xd7,0x60); //lj_test 0x60

    //HEQ
    SP0A19_write_cmos_sensor(0xfd,0x00);
    SP0A19_write_cmos_sensor(0xdc,0x00);
    SP0A19_write_cmos_sensor(0xdd,0x80); //0x80 0813  // 0x78
    SP0A19_write_cmos_sensor(0xde,0xb0); //80  0x88  0813
...
UINT32 SP0A19Open(void)
{
	...
	if(fromsd_1 == 1)//是否从SD读取//gepeiwei   120903
	{
...
BOOL SP0A19_set_param_wb(UINT16 para)
{
    ...
    case AWB_MODE_AUTO:
    {
        //SP0A19_reg_WB_auto         \D7远\AF
        SP0A19_write_cmos_sensor(0xfd,0x01);
        SP0A19_write_cmos_sensor(0x28,0xb8);//c4 yc 2014-04-01
    ...
    {
        // SP0A19_reg_WB_auto \B0壮\E3\B5\C6
    ...
    {
        //SP0A19_reg_WB_auto  荧\B9\E2\B5\C6
    ...
    {
        // SP0A19_reg_WB_auto \B0\D7\C8裙\E2
