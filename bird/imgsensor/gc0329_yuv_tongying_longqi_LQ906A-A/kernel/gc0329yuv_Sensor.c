 ...
 * Revision 1.1  2013/09/11 07:41:56  lichangying
 * Editor:lichangying
 * Inspector:lichangying
 * Reason:update for 13C43_3476 camera
 *
 * Revision 1.5  2013/08/16 08:34:23  zhoushaohong
 * 优化效果
 *
 * Revision 1.2  2013/07/29 04:24:47  wangqiang
 * 13C43_3275
 * 0329优化
 *
 * Revision 1.1  2013/07/24 06:45:10  lijunchao
 * Commit for 3275
 *
 * Revision 1.1  2013/07/16 01:17:56  lijunchao
 * commit gc0329 yuv jinlangxin for T56A
 *
 * Revision 1.1  2013/06/08 12:43:38  wangqiang
 * no message
 *
...
void GC0329NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		GC0329_write_cmos_sensor(0xfe, 0x00);
		GC0329GammaSelect(GC0329_RGB_Gamma_m3);//m5 betty
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x76, 0x80);//channel_gain_b  94 pheobe
	...
	GC0329_write_cmos_sensor(0x1f, 0xc8);//c0 pheobe
	...
	////////////////////INTPEE////////////////////
	GC0329_write_cmos_sensor(0x95, 0x54);//45 betty
	...
	GC0329_write_cmos_sensor(0x12, 0x17);//07 betty
	GC0329_write_cmos_sensor(0x13, 0x58);
  ...
  GC0329_write_cmos_sensor(0x29, 0x00);   //anti-flicker step [11:8]
	...
	GC0329_write_cmos_sensor(0x51, 0x30);//20 pheobe y_to_c
	GC0329_write_cmos_sensor(0x52, 0x0b);
	GC0329_write_cmos_sensor(0x53, 0x18);//20 pheobe inter
	GC0329_write_cmos_sensor(0x54, 0x38);//40 pheobe  inter2
	...
	GC0329_write_cmos_sensor(0x5c, 0x33);//[2] big_c   [1]dark mode  [0]block move mode  pheobe
	GC0329_write_cmos_sensor(0x5d, 0x76);//AWB_margin  pheobe  73
	...
	GC0329_write_cmos_sensor(0x69, 0x78);//85 AWB_B_gain_limit pheobe
...
void GC0329_Write_More_Registers(void)
{
	...
	GC0329_write_cmos_sensor(0x51,0x30);		//28 pheobe y_to_c																																																																							
	...
	GC0329_write_cmos_sensor(0x5c,0x33);	//35  pheobe																																																																								
	GC0329_write_cmos_sensor(0x5d,0x76);	//72 margin   pheobe																																																																								
	GC0329_write_cmos_sensor(0x67,0x80);																																																																									
	GC0329_write_cmos_sensor(0x68,0x60);	//AWB g gain limit																																																																								
	GC0329_write_cmos_sensor(0x69,0x78);		//90 AWB_B_gain_limit		  pheobe																																																																					
	...
	GC0329_write_cmos_sensor(0xfe,0x00);	
	///////////////////////////////////////////////
    	GC0329GammaSelect(2);//0:use default
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		...
		case AE_FLICKER_MODE_60HZ:
			...
			GC0329_write_cmos_sensor(0x07, 0x00);
			GC0329_write_cmos_sensor(0x08, 0x0c);
			
			GC0329_SET_PAGE1;
			GC0329_write_cmos_sensor(0x29, 0x00);   //anti-flicker step [11:8]
			GC0329_write_cmos_sensor(0x2a, 0x32);   //anti-flicker step [7:0]
			
			GC0329_write_cmos_sensor(0x2b, 0x01);   //exp level 0  15.00fps
			GC0329_write_cmos_sensor(0x2c, 0xf4); 
			GC0329_write_cmos_sensor(0x2d, 0x02);   //exp level 0  12.00fps
			GC0329_write_cmos_sensor(0x2e, 0x58); 
			GC0329_write_cmos_sensor(0x2f, 0x02);   //exp level 0  10.00fps
			GC0329_write_cmos_sensor(0x30, 0xee); 
...
BOOL GC0329_set_param_exposure(UINT16 para)
{
	...
	value_luma = (GC0329_NIGHT_MODE?0x2b:0x03);//00 betty
	value_Y = (GC0329_NIGHT_MODE?0x68:0x58);//50 betty
