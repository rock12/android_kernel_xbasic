 ...
 * Revision 1.1  2013/09/25 05:10:15  wangqiang
...
void GC0328NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		GC0328_write_cmos_sensor(0xfe, 0x00);
		GC0328GammaSelect(GC0328_RGB_Gamma_m3);
...
void GC0328_Sensor_Init(void)
{
	...
	GC0328_write_cmos_sensor(0x72 , 0x24);//18   
...
#else		
	GC0328_write_cmos_sensor(0x17 , 0x14);    //mirror//0x17 hyper
#endif		
...
#else
  GC0328_write_cmos_sensor(0x44 , 0x00);//0x hyper
#endif
	...
	GC0328_write_cmos_sensor(0x95 , 0x45);    
	
	///////YCP
	GC0328_write_cmos_sensor(0xd1 , 0x32);//32
	GC0328_write_cmos_sensor(0xd2 , 0x32);//32
	GC0328_write_cmos_sensor(0xd3 , 0x40);//32
	...
	GC0328_write_cmos_sensor(0x12 , 0x40);   
...
{
    GC0328GammaSelect(GC0328_RGB_Gamma_m3);//0:use default//m4 hyper
}
...
BOOL GC0328_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0328_write_cmos_sensor(0x30, 0xe0); //3c0
			GC0328_write_cmos_sensor(0x31, 0x07);   //exp level 3  7.14fps
			GC0328_write_cmos_sensor(0x32, 0x80); //540
			GC0328_SET_PAGE0;
			break;
		...
		case AE_FLICKER_MODE_60HZ:
			...
			GC0328_write_cmos_sensor(0x30, 0xe0); //3a8
			GC0328_write_cmos_sensor(0x31, 0x07);   //exp level 0  7.05fps
			GC0328_write_cmos_sensor(0x32, 0x80); //52e
			GC0328_SET_PAGE0;
		break;
		default:
		return FALSE;
...
BOOL GC0328_set_param_exposure(UINT16 para)
{
	switch (para)
	{
		...
		case AE_EV_COMP_00:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0x60);//80
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
