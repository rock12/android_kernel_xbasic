...
static void T8EV5YUV_Sensor_Init_setting(void)
{
    ...
    T8EV5YUV_write_cmos_sensor(0x020A,0x12); //-/GAM01P[6:0]
    T8EV5YUV_write_cmos_sensor(0x020B,0x16); //-/GAM02P[6:0]
	...
	T8EV5YUV_write_cmos_sensor(0x023A,0x04);//SHP_PGAIN1[3:0]/SHP_PGAIN0[3:0]
    T8EV5YUV_write_cmos_sensor(0x023B,0x00); //-/-/-/-/SHP_PGAIN2[3:0]
	T8EV5YUV_write_cmos_sensor(0x023C,0x04);//SHP_MGAIN1[3:0]/SHP_MGAIN0[3:0]
    ...
    T8EV5YUV_write_cmos_sensor(0x024A,0x11); //BLK_CONT1[3:0]/BLK_CONT0[3:0]
    ...
    T8EV5YUV_write_cmos_sensor(0x0303,0x00); //ALCAIM[7:0]
    ...
    T8EV5YUV_write_cmos_sensor(0x0413,0x44); //NSGAIN1[3:0]/NSGAIN0[3:0]
	...
	T8EV5YUV_write_cmos_sensor(0x042F,0x75);//;LSALUR[7:0]
	...
	T8EV5YUV_write_cmos_sensor(0x043A,0xD2);//;LSBLG[7:0] 2nd left
	...
	T8EV5YUV_write_cmos_sensor(0x043E,0xBA);//;LSBRR[7:0]//De
	T8EV5YUV_write_cmos_sensor(0x043F,0x5A);//;LSBRB[7:0]
	T8EV5YUV_write_cmos_sensor(0x0440,0xD0);//;LSCUG[7:0] 2nd up  
	T8EV5YUV_write_cmos_sensor(0x0441,0xCA);//;LSCUR[7:0] //F7
	T8EV5YUV_write_cmos_sensor(0x0442,0x8A);//;LSCUB[7:0]
	T8EV5YUV_write_cmos_sensor(0x0443,0x92);//;LSCDG[7:0] 2nd down
	T8EV5YUV_write_cmos_sensor(0x0444,0x95);//;LSCDR[7:0] 
	T8EV5YUV_write_cmos_sensor(0x0445,0x80);//;LSCDB[7:0]
...
void T8EV5YUV_set_1280_960_init(void)
{
	...
	T8EV5YUV_write_cmos_sensor(0x0229,0x2B);//Cbr_MGAIN1[3:0]/Cbr_MGAIN0[3:0]
    T8EV5YUV_write_cmos_sensor(0x0305,0x01);//-/-/-/-/AGMAX[11:8]
    T8EV5YUV_write_cmos_sensor(0x0306,0x9B);//AGMAX[7:0]
    T8EV5YUV_write_cmos_sensor(0x0307,0x0F);//MES[15:8]
    T8EV5YUV_write_cmos_sensor(0x0315,0x08);//FAUTO/FCOUNT[2:0]/FCLSBON/EXPLIM[2:0]//0x98
    T8EV5YUV_write_cmos_sensor(0x0316,0x6C);//FLLONGON/FRMSPD[1:0]/FL600S[12:8]
    T8EV5YUV_write_cmos_sensor(0x0317,0x68);//FL600S[7:0]
    T8EV5YUV_write_cmos_sensor(0x0102,0x02);//-/-/-/-/-/-/VLAT_ON/GROUP_HOLD
...
static void T8EV5_FOCUS_Init(void)
{
	...
	T8EV5YUV_write_cmos_sensor(0x2402,0x00);//;
	T8EV5YUV_write_cmos_sensor(0x2403,0x00);//;
    T8EV5YUV_write_cmos_sensor(0x2404,0x20);//;
...
static void T8EV5_FOCUS_Single_Focus()
{
	...
	T8EV5_FOCUS_Get_CPU_ST();
	#endif
	//zh add delay for touch AF with flash
	if(is_flash_on)
	{
		msleep(200);
	}
	//zh modify for touch af can't stop 20130822
...
void T8EV5YUV_set_capture_shutter_gain(UINT16 para)
{
   ...
   if(sceneMode==SCENE_MODE_HDR)
   {
	...
	#define H_PV (0x110)
	#define V_PV (0x7C*8)
	#define PCLK_PV 1   //648
			
	#define H_CP (0x15F)
	#define V_CP (0xF7*8)
	#define PCLK_CP 1   //432
	#define AG_1X 0x38
