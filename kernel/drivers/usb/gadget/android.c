...
#ifdef BIRD_USB_STRING
#define PRODUCT_STRING BIRD_USB_STRING
#else
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	// dinghongzhen modified USB string begin @20130513
	#ifdef BIRD_PRODUCT_STRING
  // replace __ with whitespace
  char USBname[256];
  memset(USBname, 0, sizeof(USBname));
  strncpy(USBname, BIRD_PRODUCT_STRING, sizeof(USBname) - 1);
   char *foundStr;
   do {
       foundStr = strstr(USBname, "__");
       if (foundStr) {
           *foundStr = ' ';
           strcpy(foundStr+1, foundStr+2);
           }
       } while (foundStr);			
	strncpy(product_string, USBname, sizeof(product_string) - 1);
	#elif defined(BIRD_USB_DRIVER_STRING)
  // replace __ with whitespace
  char USBname[256];
  memset(USBname, 0, sizeof(USBname));
  strncpy(USBname, BIRD_USB_DRIVER_STRING, sizeof(USBname) - 1);
   char *foundStr;
   do {
       foundStr = strstr(USBname, "__");
       if (foundStr) {
	...
	#endif
	// dinghongzhen modified USB string end @20130513
