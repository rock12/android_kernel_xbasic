...
struct platform_device SANLED_marquee_driver = {
	.name	       = "SANLED",
	.id            = -1,
};

#if  1//defined (CONFIG_BIRD_MARQUEE)

struct platform_device SN3216_marquee_driver = {
	.name	       = "marquee",
	.id            = -1,
};
#endif
...
struct platform_device sensor_tablet = {
	.name	       = "waltop",
	.id            = -1,
};

struct platform_device sensor_bleds = {
	.name	       = "marquee",
	.id            = -1,
};
...
#if defined(MTK_HALL_SUPPORT)
struct platform_device mt65xx_hall_driver = {
	.name	       = "mt65xx_hall",
	.id            = -1,
};
#endif
...
#if !defined(BIRD_TP_DRIVER_LOAD_AFTER_SENSOR)
...
#if defined(CUSTOM_KERNEL_BLEDS)
	retval = platform_device_register(&sensor_bleds);
		printk("\nMYCAT sensor_bleds device! retval=%d\n",retval);
	if (retval != 0)
		return retval;
#endif
...
//=======================================================================
	...
	retval = platform_device_register(&SANLED_marquee_driver);
	printk("HMH SANLED sensor_marquee device!");
	if (retval != 0)
	{
	  printk("HMH SANLED sensor_marquee device 1111 error!");
		return retval;
	}

#if  1 //defined(CONFIG_BIRD_MARQUEE)
	retval = platform_device_register(&SN3216_marquee_driver);
	printk("HMH MARQUEE sensor_marquee device!");
	if (retval != 0)
	{
	       printk("HMH MARQUEE sensor_marquee device 1111 error!");
		return retval;
	}
#endif
...
#if defined(MTK_HALL_SUPPORT)
	retval = platform_device_register(&mt65xx_hall_driver);    
	if (retval != 0) {       
		printk("hmhhall mt65xx_hall_driver register ret %d", retval);
		return retval;    
	}
#endif
