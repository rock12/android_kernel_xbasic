...
#include <linux/aee.h>
#if defined (BIRD_HAS_FLIP_CLAM)
#define KPD_HAS_FLIP_FUN KPD_YES
#else
#define KPD_HAS_FLIP_FUN KPD_NO
#endif
...
#define SET_BIRD_VALIDATE_ACTIVE	_IOR('k', 31, int)
...
static bool kpd_flip_state = !CUST_EINT_MHALL_POLARITY;
#endif
...
static void kpd_flip_handler(unsigned long data)
{
    ...
    Mhall_key_level = mt_get_gpio_in(GPIO145);
    ...
    if (kpd_flip_state)
    {
		...
		kpd_print("mycat report KEY_FLIP=%d, flip state= %d\n", KEY_FLIP_OPEN,old_state);
    }
    else
    {
...
static long kpd_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int temp_int;// bird for validate temp variable
int Mhall_key_level;

	switch (cmd) {
#if KPD_HAS_FLIP_FUN
                    case GET_KPD_FLIP_KEY:
                        
                        Mhall_key_level = mt_get_gpio_in(GPIO145);
                        if (copy_to_user((void*)arg, &Mhall_key_level, sizeof(int)))
                            return -EFAULT;
	...
	case SET_KPD_KCOL:
		...
		break;
	case SET_BIRD_VALIDATE_ACTIVE:
		//kpd_print("kpd_dev_ioctl,    ---------------------SET_BIRD_VALIDATE_ACTIVE\n");
		temp_int = 1;
		if (copy_to_user((void*)arg, &temp_int, sizeof(int)))
			return -EFAULT;
		break;
...
#if KPD_HAS_FLIP_FUN
    mt65xx_eint_set_polarity(CUST_EINT_MHALL_NUM, CUST_EINT_MHALL_SENSITIVE);
    mt65xx_eint_set_hw_debounce(CUST_EINT_MHALL_NUM,
                                CUST_EINT_MHALL_DEBOUNCE_CN);
    mt65xx_eint_registration(CUST_EINT_MHALL_NUM, true, CUST_EINT_MHALL_POLARITY, kpd_flip_eint_handler, false);
    mt65xx_eint_unmask(CUST_EINT_MHALL_NUM);
#endif
