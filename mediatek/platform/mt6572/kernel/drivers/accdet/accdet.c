...
void accdet_eint_work_callback(struct work_struct *work)
{
   //KE under fastly plug in and plug out
	...
	ACCDET_DEBUG("[Accdet_plug in] TOP_CKPDN=0x%x!\n", pmic_pwrap_read(TOP_CKPDN));
...
static inline void clear_accdet_interrupt(void)
{
	...
	ACCDET_DEBUG("[Accdet]after clear_accdet_interrupt: ACCDET_IRQ_STS = 0x%x\n", pmic_pwrap_read(ACCDET_IRQ_STS));
}
...
*/
        ...
        {
            //send key event
#ifdef BIRD_HEASET_BUTTON_PLAYPAUSE
			if(call_status !=0){//ON CALL
        		ACCDET_DEBUG("[Accdet][key]wqcat sendKeyEvent call_status=%d, accdet_key_event=%d\n", call_status, accdet_key_event);
        		if(ACC_ANSWER_CALL == accdet_key_event){
	                ACCDET_DEBUG("[Accdet][key]wqcat answer call!\n");
	                input_report_key(kpd_accdet_dev, KEY_CALL, 1);
	                input_report_key(kpd_accdet_dev, KEY_CALL, 0);
	                input_sync(kpd_accdet_dev);
	            	}
	            if(ACC_END_CALL == accdet_key_event){
	                ACCDET_DEBUG("[Accdet][key]wqcat end call!\n");
        		...
        		if(ACC_MEDIA_PLAYPAUSE == accdet_key_event){
					ACCDET_DEBUG("[Accdet][key]wqcat music pause!\n");
...
#define PIN_ADC_CHANNEL 5
	...
	switch(accdet_status)
    {
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
			...
			if(call_status != 0) 
			{
				ACCDET_DEBUG("[Accdet]long press detect \n");
				if(is_long_press())
				{
				...
				// call is idle. control media play /pause
				if (!is_long_press()){
#ifdef BIRD_HEASET_BUTTON_PLAYPAUSE
					ACCDET_DEBUG("[Accdet]wqcat send short press remote button event %d\n",ACC_MEDIA_PLAYPAUSE);
					notify_sendKeyEvent(ACC_MEDIA_PLAYPAUSE);
#else
					ACCDET_DEBUG("[Accdet]wqcat send short press remote button event %d\n",ACC_ANSWER_CALL);//for mmi test when BIRD_HEASET_BUTTON_PLAYPAUSE is not defined by wangqiang 20140207
					notify_sendKeyEvent(ACC_ANSWER_CALL);					
#endif
				}
			}
