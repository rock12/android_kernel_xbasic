...
//huangrunming b
#ifdef BIRD_LOGO_READ_WRITE
#define AEE_IPANIC_LOGO   "logo"
#define NAND_PAGE_SIZE    (2048) 

static struct mtd_info *logobin_mtd=NULL;

void save_logo_mtd(struct mtd_info *mtd){
	printk("save_logo_mtd\n");

	
	logobin_mtd=mtd;
}

int card_bin_read(unsigned char* buf, unsigned int len, unsigned int offset){
	unsigned int retlen;

	printk("\n card_bin_read logobin_mtd=%d\n",logobin_mtd);		

	if (logobin_mtd!=NULL){	
		logobin_mtd->_read(logobin_mtd, offset, len, &retlen, buf);
	}
	else{
		printk("logobin_mtd not init\n");		
...
int card_bin_write(unsigned char* buf, unsigned int len, unsigned int offset){
	
	unsigned int retlen,addr,length,ret;
	unsigned char *buffer,*p;
	struct erase_info erase;


	
	printk("\n card_bin_write logobin_mtd=%d \n",logobin_mtd);		
	...
	buffer=kmalloc(NAND_PAGE_SIZE,GFP_KERNEL);
	...
	erase.mtd=logobin_mtd;
	erase.addr=0x2e0000;
	erase.len=0x20000;  //128k
	ret=logobin_mtd->_erase(logobin_mtd, &erase);
	printk("erase ret=%d,state=%d\n",ret,erase.state);

	logobin_mtd->_write(logobin_mtd, offset, len, &retlen, buffer);
...
//huangrunming b
#ifdef BIRD_LOGO_READ_WRITE
	printk("mtd_panic_notify_add\n");


	if (strcmp(mtd->name,AEE_IPANIC_LOGO)==0){
