 ...
 * $Log: linear_charging.c,v $
 * Revision 1.3  2013/07/29 06:33:55  birdlibo
 * no message
 *
 * Revision 1.2  2013/07/29 02:46:03  birdlibo
 * no message
 *
 * Revision 1.1  2013/07/05 03:12:30  maliejun
 * BIRD_HIGH_OR_NOT_BATTERY_COMPATIBLE = no
 *
 *             HISTORY
...
extern unsigned int g_BatteryType;
#if defined(BIRD_NTC_PIN3_BATTERY_COMPATIBLE)
extern unsigned int g_Ntc_Pin3_BatteryType;
#endif
...
#if defined(BIRD_CHARGING_TEMPERATURE_CONTROL)

void set_usb_charging_current(void)
{
...
//		g_temp_input_CC_value = CHARGE_CURRENT_300_00_MA;
	...
	}else if(BMT_status.temperature>=11&&BMT_status.temperature<=50){
...
//		g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
		...
		battery_xlog_printk(BAT_LOG_CRTI, "mycat [BATTERY] usb BMT_status.temperature=%d\r\n", BMT_status.temperature);
}

void set_ac_charging_current(void){
	if(BMT_status.temperature>=0&&BMT_status.temperature<=12){
//		g_temp_input_CC_value = CHARGE_CURRENT_200_00_MA;
		g_temp_CC_value = CHARGE_CURRENT_200_00_MA;
	}else if(BMT_status.temperature>=13&&BMT_status.temperature<=23){
//		g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
		g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
	}else if(BMT_status.temperature>=24&&BMT_status.temperature<=45){
//		g_temp_input_CC_value = CHARGE_CURRENT_800_00_MA;
		g_temp_CC_value = CHARGE_CURRENT_800_00_MA;
	}else if(BMT_status.temperature>=46&&BMT_status.temperature<=50){
//		g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
		g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
	}else{
//		g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
		...
		battery_xlog_printk(BAT_LOG_CRTI, "mycat [BATTERY] ac BMT_status.temperature=%d\r\n", BMT_status.temperature);
}
#endif
...
void select_charging_curret(void)
{
    ...
    {    
    		//modify by will
if(BMT_status.bat_vol>=4280)//4350
{

printk("mycat select_charging_curret++++ \r\n");
g_temp_CC_value=USB_CHARGER_CURRENT;//450Ma
...
static void pchr_turn_on_charging (void)
{
	...
	kal_uint32 charging_enable2 = KAL_FALSE;

	battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] pchr_turn_on_charging()!\r\n");
	
#if defined(BIRD_HIGH_OR_NOT_BATTERY_COMPATIBLE)  
	if(g_BatteryType)//added by maliejun 20130704 普通电池不充电
	{
		printk("mycat pchr_turn_on_charging stop \r\n");
		battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable2);
		return;
	}
	else
	{
		printk("mycat pchr_turn_on_charging start \r\n");
	}
#endif
    ...
    else
    {
        ...
        {    
			...
			/* Set CV */
			...
			#if defined(BIRD_NTC_PIN3_BATTERY_COMPATIBLE)
			 printk("mycat pchr_turn_on_charging g_Ntc_Pin3_BatteryType=%d \r\n",g_Ntc_Pin3_BatteryType);
			 ...
			 else
			 {
					...
					cv_voltage = BATTERY_VOLT_04_300000_V;//BATTERY_VOLT_04_350000_V
				#else
		            cv_voltage = BATTERY_VOLT_04_200000_V;
				#endif
			#endif
...
#if defined(BIRD_NTC_PIN3_BATTERY_COMPATIBLE)
	 printk("mycat BAT_ConstantCurrentModeAction g_Ntc_Pin3_BatteryType=%d \r\n",g_Ntc_Pin3_BatteryType);
	 if(g_Ntc_Pin3_BatteryType)//4.35v
	 {
	 		v_cc2topoff_threshold = V_CC2TOPOFF_THRES_4_35V; 
...
#if defined(BIRD_NTC_PIN3_BATTERY_COMPATIBLE)
	   printk("mycat BAT_BatteryFullAction g_Ntc_Pin3_BatteryType=%d \r\n",g_Ntc_Pin3_BatteryType);
		 if(g_Ntc_Pin3_BatteryType)//4.35v
		 {
		 		if(BMT_status.bat_vol < RECHARGING_VOLTAGE_4_35V )         	
		    {
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
    ...
    /*  Disable charger */
	//modify by caikuowei for recharging when the Charger_vol below the max begin 
	if(BMT_status.temperature > -19 && BMT_status.charger_protect_status ==BATTERY_UNDER_TEMPMIN)
	{
		BMT_status.bat_charging_state = CHR_CC;
