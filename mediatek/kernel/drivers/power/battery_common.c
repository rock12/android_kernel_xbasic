...
unsigned int g_BatteryType=0x0;//"0" 表示高压电池，"1" 表示普通电池
unsigned int g_Ntc_Pin3_BatteryType=0x0;//"0" 表示4.2v电池，"1" 表示4.35v电池
...
kal_bool upmu_is_chr_det(void)
{
    ...
    if(tmp32 == 0)
    {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] upmu_is_chr_det  \r\n");
        if (is_fan5402_ac_charge()==true
            &&is_fan5402_ac_charge_LV()==true){
...
{	
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] kernel mt_battery_CheckBatteryTemp = %d \r\n",BMT_status.temperature); 
...
#if defined(CHARGER_AW3215_SUPPORT)
    mt_set_gpio_mode(GPIO_CHR_EN_PIN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_CHR_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHR_EN_PIN, GPIO_OUT_ONE);

    mt_set_gpio_mode(GPIO_CHR_STA_PIN, GPIO_MODE_00);        //CHARGE STOP INFO
    mt_set_gpio_dir(GPIO_CHR_STA_PIN, GPIO_DIR_IN);

    mt_set_gpio_mode(GPIO_CHR_CTRL_PIN, GPIO_MODE_00);        //OTG STOP
    mt_set_gpio_dir(GPIO_CHR_CTRL_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHR_CTRL_PIN, GPIO_OUT_ZERO);
#endif            
        status = PMU_STATUS_FAIL;       
    }   
	
	//add for when the temp under -20 stop charging begin
#if !defined(BIRD_SAMSUMG_STYLE_NTC_CTL)
	else if (BMT_status.temperature < -19)
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]Charger temp under -20\r\n"); 
		BMT_status.bat_charging_state = CHR_ERROR;
		BMT_status.charger_protect_status = BATTERY_UNDER_TEMPMIN; 
		status = PMU_STATUS_FAIL; 
...
static PMU_STATUS mt_battery_CheckCallState(void)
{
	...
	#if defined(BIRD_NTC_PIN3_BATTERY_COMPATIBLE)
	 printk("mycat mt_battery_CheckCallState g_Ntc_Pin3_BatteryType=%d \r\n",g_Ntc_Pin3_BatteryType);
	 if(g_Ntc_Pin3_BatteryType)//4.35v
	 {
	 		if((g_call_state == CALL_ACTIVE) && (BMT_status.bat_vol > V_CC2TOPOFF_THRES_4_35V))
				status = PMU_STATUS_FAIL;
	 }
	 else
	 {
...
static void mt_battery_charger_detect_check(void)
{
    ...
    else 
    {
        ...
        mt_usb_disconnect();          
#if defined(BIRD_FAN540X_SUPPORT)
        BATTERY_ResetFan5402DetectStatus();
#endif
    }
}
...
#if defined(CHARGER_AW3215_SUPPORT)     //whj add for charge real stop read state

char  PreAWINTState = 0;
char  CurAWINTState = 0;
char  AWINTDebunce= 0;
char  AW3215_ChargeStop = 0;

char  checkHW_ChargeStop()
{
    int gpiostate;
    char ret = 0;
    
    gpiostate = mt_get_gpio_in(GPIO_CHR_STA_PIN);

    printk("Power/Battery AW3215 int state = %d\n", gpiostate);

#if 0
    CurAWINTState = gpiostate;
    
    if(PreAWINTState == 0 && CurAWINTState == 1)
    {
        AWINTDebunce = 1;
    }

    if(AWINTDebunce == 1 && CurAWINTState == 1)
    {
        AW3215_ChargeStop = 1;
    }

    PreAWINTState == gpiostate;
#endif
    if(gpiostate == 1)
    {
        if(AWINTDebunce == 0)
...
#if defined(BIRD_HIGH_OR_NOT_BATTERY_COMPATIBLE)       	
		 		batteryV = battery_meter_get_tempV();//added by maliejun 20130704
				printk("mycat BAT_thread batteryV=%d \r\n",batteryV);
				//battery_charging_control(CHARGING_CMD_ENABLE,0);
				if(batteryV >= 50)// 普通电池 mV
				{
					//arch_reset(0,NULL);
					g_BatteryType = 1;
				}
...
//unsigned int g_Ntc_Pin3_BatteryType=0x0;//"0" 表示4.2v电池，"1" 表示4.35v电池    	
				...
				printk("mycat BAT_thread batteryV=%d \r\n",batteryV);
				//battery_charging_control(CHARGING_CMD_ENABLE,0);
				if(batteryV <= 200)// 4.2v电池
				{
					g_Ntc_Pin3_BatteryType = 0;
				}
				else
				{
					g_Ntc_Pin3_BatteryType = 1;
				}
#endif
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    mt_set_gpio_mode(GPIO_CHR_EN_PIN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_CHR_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHR_EN_PIN, GPIO_OUT_ZERO);
...
static int __init battery_init(void)
{
    ...
    kal_int32 batteryV;
		...
		//batteryV = battery_meter_get_tempV();
		//printk("mycat battery_init batteryV=%d \r\n",batteryV);
