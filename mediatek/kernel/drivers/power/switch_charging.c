 ...
 * $Log: switch_charging.c,v $
 * Revision 1.1  2013/08/05 03:01:04  shenzhiwang
 * Editor: shenzhiwang
 * Inspector:
 * Reason:72 P24-P32
 *
...
#if defined(BIRD_FAN540X_SUPPORT)

CHR_CURRENT_ENUM g_fan5402_CC_value = FAN5402_AC_CC_FIRST;
CHR_CURRENT_ENUM g_fan5402_input_CC_value = FAN5402_AC_CC_FIRST;
kal_uint32	g_fan5402_detect_count=0;

void BATTERY_ResetFan5402DetectStatus(void){
    g_fan5402_CC_value = FAN5402_AC_CC_FIRST;
    g_fan5402_input_CC_value = FAN5402_AC_CC_FIRST;
    g_fan5402_detect_count=0;
	
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BATTERY_ResetFan5402DetectStatus\r\n");
	
}
void BATTERY_SetFan5402Current(void){
	CHR_CURRENT_ENUM c;

	g_fan5402_detect_count=-1;
	
	c=g_fan5402_CC_value-FAN5402_AC_CC_STEP;
	
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] 5402 & ac charger & upmu %d \r\n",c);
	
	battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,&c);
	battery_charging_control(CHARGING_CMD_SET_CURRENT,&c);

}

kal_bool is_fan5402_ac_charge(void){
	if(is_pmic_fan5405()==false
...
kal_bool is_fan5402_ac_charge_LV(void){
	
	kal_int32 ret_data=0;
	ret_data=PMIC_IMM_GetOneChannelValue(VCHARGER_CHANNEL_NUMBER,10,1);
	
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] fan5402 vchg = %d,g_fan5402_CC_value=%d \r\n", ret_data,g_fan5402_CC_value);

	
	if(ret_data>FAN5402_VCDT_LV&&ret_data<FAN5402_VCDT_HV
           &&g_fan5402_CC_value>FAN5402_AC_CC_FIRST){
                return true;
	}
	else{
...
void select_charging_curret(void)
{
    ...
    {    
...
#if defined(BIRD_FAN540X_SUPPORT)
        {
            ...
            ret_data=PMIC_IMM_GetOneChannelValue(VCHARGER_CHANNEL_NUMBER,10,1);
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] fan5402 vchg = %d\r\n", ret_data);

            if (is_fan5402_ac_charge()==true&&g_fan5402_detect_count!=-1){

                //if (g_fan5402_detect_count%2==0 )
                {

                    if (ret_data>FAN5402_VCDT_HV){
                        if (g_fan5402_input_CC_value + FAN5402_AC_CC_STEP< g_temp_input_CC_value) {
                            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] fan5402 cc add current \r\n");
                            g_fan5402_input_CC_value=FAN5402_AC_CC_STEP*g_fan5402_detect_count+FAN5402_AC_CC_FIRST;
                            g_fan5402_CC_value=g_fan5402_input_CC_value;
                        }else{
                            if (g_fan5402_input_CC_value< g_temp_input_CC_value){
                                g_fan5402_input_CC_value=g_temp_input_CC_value;
                                g_fan5402_CC_value=g_temp_CC_value;
                            }
				...
				battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] fan5402 cc mode charging : %d, input current = %d,count=%d \r\n", g_fan5402_CC_value,g_fan5402_input_CC_value,g_fan5402_detect_count);
				
                g_fan5402_detect_count++;
            }
        }
#endif
#if defined(MTK_JEITA_STANDARD_SUPPORT)
...
static void pchr_turn_on_charging (void)
{
    ...
    else
    {
        ...
        {    
#if defined(BIRD_FAN540X_SUPPORT)
            if (is_fan5402_ac_charge()==true){

                battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] 5402 & ac charger \r\n");
				if (is_fan5402_ac_charge_LV()==true){
						BATTERY_SetFan5402Current();
				}else{
                	battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,&g_fan5402_input_CC_value);
	                battery_charging_control(CHARGING_CMD_SET_CURRENT,&g_fan5402_CC_value);
				}
			...
			/*Set CV Voltage*/
                ...
                #else
			      cv_voltage = BATTERY_VOLT_04_220000_V;
               #endif
			#endif
