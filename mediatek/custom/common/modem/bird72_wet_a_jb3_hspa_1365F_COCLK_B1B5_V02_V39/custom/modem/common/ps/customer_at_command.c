...
// add by yangbinjie for reset sim lock 20130826
kal_bool reset_sim_auto_lock(void);
...
kal_bool custom_command_hdlr(char *full_cmd_string)
{
    ...
    #endif
    else if(strcmp(cmd_name, "RESETSIMLOCK") == 0)
    {
        // add by yangbinjie for reset sim lock 20130826
		
        kal_prompt_trace(stack_get_active_module_id(),  "yangbinjie RESET SIM LOCK");

        kal_bool result =  reset_sim_auto_lock();
...
{
	kal_bool read, write;
	nvram_sml_context_struct buff;
	sml_category_meta_struct cat[SML_SUPPORT_CAT_SIZE];

	kal_uint32 SIM_LOCK_INDEX = 4;

	read=nvram_external_read_data(NVRAM_EF_SML_LID,1,(kal_uint8*) &buff, sizeof(nvram_sml_context_struct));       
	...
	buff.cat[SIM_LOCK_INDEX].state = SML_STATE_AUTOLOCK;
	buff.cat[SIM_LOCK_INDEX].autolock_count = 1;
	buff.cat[SIM_LOCK_INDEX].num = 0;

	write = nvram_external_write_data(NVRAM_EF_SML_LID, 1, (kal_uint8 *)& buff,sizeof(nvram_sml_context_struct));
