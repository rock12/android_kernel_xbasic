...
static int msensor_test_i2c(void)
{
	unsigned char i2c_data[16] = {0};
	i2c_data[0] = MMC3416X_REG_CTRL;
	i2c_data[1] = MMC3416X_CTRL_REFILL;
	if(I2C_TxData(i2c_data, 2) < 0)
	{
		printk(KERN_ERR "msensor_test_i2c,I2C_TxData failed\n");
		return -1;
	}	
...

{
	...
	switch (cmd)
	{
		...
		case MSENSOR_IOCTL_READ_SENSORDATA:	
			...
			break;
		//huangrunming b
		case MSENSOR_IOCTL_TEST_I2C:
			status = msensor_test_i2c();
			printk(KERN_ERR "MSENSOR_IOCTL_TEST_I2C, status=%d!\r\n",status);
			if(copy_to_user(argp, &status, sizeof(status)))
			{
				MMCDBG("copy_to_user failed.");
				return -EFAULT;
