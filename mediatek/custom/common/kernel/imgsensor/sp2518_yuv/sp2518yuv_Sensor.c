 ...
 *------------------------------------------------------------------------------
 * $Log: sp2518yuv_Sensor.c,v $
 * Revision 1.1  2013/11/01 05:40:03  maliejun
 * 13C43-T5 调天线软件安排
 *
...
void SP2518_night_mode(kal_bool bEnable)
{
	...
	if (bEnable)/*night mode settings*/
	{
       	...
       	if(SP2518_MPEG4_encode_mode == KAL_TRUE) /*video night mode*/
		{
			...
			{	
				#if 0
				//X pll   fix 6fps      video night mode 50hz                                
				SP2518_write_cmos_sensor(0xfd , 0x00);
	...
	else /*normal mode settings*/
	{
        ...
        if (SP2518_MPEG4_encode_mode == KAL_TRUE) 
		{
			...
			if(SP2518_CAM_BANDING_50HZ == KAL_TRUE)/*video normal mode 50hz*/
			{
				...
				#if 1//24M 2PLL 8.5fps fixed 50hz
				//ae setting
				SP2518_write_cmos_sensor(0xfd,0x00);
				...
				//Status                          
				SP2518_write_cmos_sensor(0xfd,0x01);
				SP2518_write_cmos_sensor(0xce,0x34);
				SP2518_write_cmos_sensor(0xcf,0x05);
				SP2518_write_cmos_sensor(0xd0,0x34);
				SP2518_write_cmos_sensor(0xd1,0x05);
			...
			else/*video normal mode 60hz*/
			{
				...
				#if 1//24M 2Pll 60hz fixed 8.5fps
				...
				//Status                          
				SP2518_write_cmos_sensor(0xfd,0x01);
				SP2518_write_cmos_sensor(0xce,0x73);
				SP2518_write_cmos_sensor(0xcf,0x05);
				SP2518_write_cmos_sensor(0xd0,0x73);
				SP2518_write_cmos_sensor(0xd1,0x05);
...
void SP2518_Sensor_Init(void)
{
	...
	SP2518_write_cmos_sensor(0x2c,0x33);//31 Ronlus remove balck dot0x45);
	...
	SP2518_write_cmos_sensor(0x0a , 0x8b);
	SP2518_write_cmos_sensor(0x2f , 0x00);
	SP2518_write_cmos_sensor(0x30 , 0x08;// 08
	SP2518_write_cmos_sensor(0xf0 , 0xa9);
	SP2518_write_cmos_sensor(0xf1 , 0x00);
	SP2518_write_cmos_sensor(0xfd , 0x01);
	SP2518_write_cmos_sensor(0x90 , 0x0c);
...
*/  
	...
	SP2518_write_cmos_sensor(0xde,0x95);//a0
...
BOOL SP2518_set_param_wb(UINT16 para)
{
	...
	switch (para)
	 {
	   case AWB_MODE_OFF:
			 //SP2518_write_cmos_sensor(0xfd,0x00);//20121025 for 全景capture
			 //SP2518_write_cmos_sensor(0x32,0x05);//20121025 for 全景capture
		 break; 					
	   case AWB_MODE_AUTO:
			 SP2518_write_cmos_sensor(0xfd,0x01);
			 SP2518_write_cmos_sensor(0x28,0xce);
...
UINT32 sp2518_get_sensor_id(UINT32 *sensorID) 
{
    ...
    *sensorID = SP2518_read_cmos_sensor(0x02);
    printk("mycat sp2518,sensor_id:%x \n",*sensorID);
