...
/*----------------------------------------------------------------------------*/
#define FTM_CUST_ALSPS "/data/ltr559"
...
{
	//struct ltr559_priv *obj = i2c_get_clientdata(client);
	printk("MYCAT ltr559_init_device.........\r\n");
	u8 buf =0;
	int i = 0;
	int ret = 0;

	hwmsen_write_byte(client, 0x82, 0x7F);				//100mA,100%,60kHz		//yaoyaoqin
	hwmsen_write_byte(client, 0x83, 0x08);				//4 pulse
	hwmsen_write_byte(client, 0x84, 0x00);				//50 ms
...
static int ltr559_init_client(struct i2c_client *client)
{
	struct ltr559_priv *obj = i2c_get_clientdata(client);
	int err=0;
	printk("MYCAT ltr559_init_client.........\r\n");
	...
	{
		printk("MYCAT ltr559_init_device error: %d\n", err);
		return err;
	}
	return err;
}
...
static int ltr559_calibrate(struct i2c_client *client)
{
	struct ltr559_priv *obj = i2c_get_clientdata(client);    
	struct file *fp;
	int ret = 0;
	int w,count_temp;
	UINT16 distance_temp;

	printk("hgw111111:ltr559_calibrate open nvram file!\r\n");
	ltr559_enable_ps(client, 1);
	msleep(100);
	ltr559_read_data_ps(client, &obj->ps);
	ltr559_enable_ps(client, 0);
	
	printk("ltr559_calibrate value is obj->ps = %d \r\n",obj->ps);
	
#ifdef BIRD_PS_CALI_ADD_VAL
	distance_temp = (obj->ps) + simple_strtol(BIRD_PS_CALI_ADD_VAL, NULL, 0);
#else
	distance_temp = (obj->ps) + 8;
#endif
	if(distance_temp<3)
		distance_temp=obj->ps_thd_val;//atomic_read(&obj->ps_thd_val);
	...
	count_temp = fp->f_op->write(fp,
								  ...
								  &fp->f_pos);	
	filp_close(fp, NULL);	
	printk("ltr559_calibrate value is count_temp = %x,distance_temp = %d\r\n",count_temp,distance_temp);
...
static int alsps_test_i2c(struct i2c_client *client)
{
	if (hwmsen_write_byte(client, 0x82, 0x7F)) {
		APS_LOG("ltr559_enable_ps enable failed!\n");
		return -1;
	}
...
static long ltr559_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case ALSPS_PS_TEST:
			if (err = ltr559_enable_ps(obj->client, true))
			{
				APS_ERR("hrm enable ps fail: %d\n", err);
				err = -EFAULT;
				goto err_out;
			}
			status = alsps_test_i2c(obj->client);
			printk(KERN_ERR "ALSPS_PS_TEST, status=%d!\r\n",status);
			if (copy_to_user(ptr, &status, sizeof(status)))
			{
				err = -EFAULT;
				goto err_out;
			}
		...
		case ALSPS_SET_PS_CALIBRATION:    
			dat = ltr559_calibrate(obj->client);
			break;			
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
	    printk("MYCAT ltr559_i2c_probe ltr559_init_client ERROR\n");
		goto exit_init_failed;
	}

	if(err = ltr559_enable_als(client, false))
	{
		APS_ERR("mycat disable als fail: %d\n", err);
	}
	if(err = ltr559_enable_ps(client, false))
	{
		APS_ERR("mycat disable ps fail: %d\n", err);
	}

	if(err = misc_register(&ltr559_device))
	{
		APS_ERR("mycat ltr559_device register failed\n");
		goto exit_misc_device_register_failed;
	}

	if(err = ltr559_create_attr(&ltr559_alsps_driver.driver))
	{
		APS_ERR("mycat create attribute err = %d\n", err);
...
{
	APS_FUN();
	printk("MYCAT ltr559_init.........\r\n");
	//wake_lock_init(&ps_wake_lock,WAKE_LOCK_SUSPEND,"ps module");
	i2c_register_board_info(1, &i2c_ltr559, 1);  //xiaoqian, 20120412, add for alsps
