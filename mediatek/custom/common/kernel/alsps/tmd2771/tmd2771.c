...
#define FTM_CUST_ALSPS "/data/tmd2771"
...
static long TMD2771_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable)
		...
		if(test_bit(CMC_BIT_ALS, &obj->enable))
			...
			databuf[1] = (u8)(atomic_read(&obj->ps_thd_val_low) & 0x00FF);//750
			res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
			...
			databuf[1] = (u8)((atomic_read(&obj->ps_thd_val_low) & 0xFF00) >> 8);//750
			res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
			...
			databuf[1] = (u8)(atomic_read(&obj->ps_thd_val_high) & 0x00FF);//900
			res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
			...
			databuf[1] = (u8)((atomic_read(&obj->ps_thd_val_high) & 0xFF00) >> 8);//900
			res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
			{
				goto EXIT_ERR;
...
void TMD2771_eint_func(void)
{
	...
	//APS_LOG(" mycat TMD2771_eint_func performed!\n");
	if(!obj)
	{
		return;
	}
...
static int TMD2771_get_ps_value(struct TMD2771_priv *obj, u16 ps)
{
    ...
    printk("mycat open file fp = 0x%x \r\n",fp);	
		...
		if(fp != 0xfffffffe)//open file suscess
		{
			...
			ps_cali.close = ((buf[0]<<8))&0xff00 | (buf[1]);
			ps_cali.far_away = ((buf[2]<<8))&0xff00 | (buf[3]);
			ps_cali.valid = 1;
			printk("mycat in file is ps_cali.close = %d ps_cali.far_away = %d ps_cali.valid = %d buf[0] = %x,buf[1] = %x,buf[2] = %x,buf[3] = %x,\n", ps_cali.close,ps_cali.far_away,ps_cali.valid,buf[0],buf[1],buf[2],buf[3]);	
		}
	...
	if(ps_cali.valid == 1)
		{ 
			...
			printk("mycat TMD2771_get_ps_value:: ps_cali.close=%d ps_cali.far_away=%d ps=%d \r\n",ps_cali.close,ps_cali.far_away,ps);
	}
	else
	{
		  ...
		  printk("mycat TMD2771_get_ps_value:: obj->ps_thd_val_high=%d obj->ps_thd_val_low=%d ps=%d \r\n",obj->ps_thd_val_high,obj->ps_thd_val_low,ps);
...
static int TMD2771_calibrate(struct i2c_client *client)
{
	struct TMD2771_priv *obj = i2c_get_clientdata(client); 
	struct file *fp;
	int ret = 0;
	...
	printk("hgw111111:TMD2771_calibrate open nvram file!\r\n");
	TMD2771_enable_ps(client, 1);
	msleep(100);
	TMD2771_read_ps(client, &obj->ps);
	TMD2771_enable_ps(client, 0);
	
	printk("TMD2771_calibrate value is obj->ps = %d \r\n",obj->ps);
	
#if defined(BIRD_PS_CALI_ADD_VAL)
	distance_temp_high = (obj->ps) + simple_strtol(BIRD_PS_CALI_ADD_VAL, NULL, 0);
#else
	distance_temp_high = (obj->ps) + 16;
#endif

#if defined(BIRD_PS_CALI_ADD_VAL2)
	distance_temp_low = (obj->ps) + simple_strtol(BIRD_PS_CALI_ADD_VAL2, NULL, 0);
#else
	distance_temp_low = (obj->ps) + 8;
#endif
	...
	count_temp = fp->f_op->write(fp,
								  ...
								  &fp->f_pos);	
	filp_close(fp, NULL);	
	printk("TMD2771_calibrate value is count_temp = %x,buf[0] = %x buf[1] = %x buf[2] = %x buf[3] = %x distance_temp_high=%x distance_temp_low=%x\r\n",count_temp,buf[0],buf[1],buf[2],buf[3],distance_temp_high,distance_temp_low);
       ...
       unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case ALSPS_SET_PS_CALIBRATION:    
			dat = TMD2771_calibrate(obj->client);
			break;
