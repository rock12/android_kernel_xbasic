...
{0xBC,3,{0x00, 0x95, 0x00}},

// Set VGMN = -4.9V / VGSN = 0V
{0xBD,3,{0x00, 0x95, 0x00}},
...
static unsigned int lcm_compare_id(void)
{
   ...
   unsigned int data_array[16];
    ...
    id = (id1<<8) | id2;
    Lcd_Log("%s, mycat linglong IC_id = 0x%x,GPIO_id=0x%x\n", __func__, id,get_lcd_id());
    return (0x5517 == id)?1:0;
//	return (0x5517 == id&&get_lcd_id()==0)?1:0;
}
...
//  }
	...
	printk("<3>mycat$$ nt35517_hongsheng_HS4661228A_HSD_dsi_qhd ### esd check buffer[1] = 0x%x\r\n",buffer[0]);
	if(buffer[0] == 0x9C)//9C
		return 0;
	else
		return 1;
...
LCM_DRIVER nt35517_hongsheng_HS4661228A_HSD_dsi_qhd_lcm_drv = 
{
	.name		= "nt35517_hongsheng_HS4661228A_HSD_dsi_qhd",
