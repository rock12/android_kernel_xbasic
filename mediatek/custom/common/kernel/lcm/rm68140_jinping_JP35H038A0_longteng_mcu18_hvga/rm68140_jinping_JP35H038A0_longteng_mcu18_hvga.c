...
{
	unsigned char i,n;
	Lcd_Log("mycat LCD rm68140_HF_140391226_A0_TM_mcu18_hvga\n");	
   ...
   //************* Start Initial Sequence **********// 
     send_ctrl_cmd(0xC0); 
    send_data_cmd(0x0A);    // P-Gamma level 
    send_data_cmd(0x0A);    // N-Gamma level 
    ...
    send_ctrl_cmd(0xC5); 
    send_data_cmd(0x00); 
    send_data_cmd(0x30);     // VCM Setting 42  set vcom
    send_data_cmd(0x80);     // VCM Register Enable 
    
    send_ctrl_cmd(0xC2);    // DC1.DC0 Setting 
    send_data_cmd(0x33);      
  	
  	send_ctrl_cmd(0xB1);      // BT & VC Setting 
    send_data_cmd(0xD0);   
    send_data_cmd(0x11);
  
		send_ctrl_cmd(0xB4); 
		send_data_cmd(0x00);     // Frame Rate Setting  0x00
		 
		send_ctrl_cmd(0xB6);      // RM.DM Setting 
		send_data_cmd(0x00); 
		send_data_cmd(0x22); //42
		send_data_cmd(0x3B); 
    ...
    send_data_cmd(0x00); 
		...
		send_data_cmd(0x08); ////08
		
		send_ctrl_cmd(0xEE);     // Color filter setting 
		send_data_cmd(0x00);
		
		send_ctrl_cmd(0x35);     //te
		send_data_cmd(0x00); 
		/*
		send_ctrl_cmd(0x44);     // Exit ENG , must be set before gamma setting 
		send_data_cmd(0x00); 
		send_data_cmd(0x01); 
    */
...
static unsigned int lcm_compare_id(void)
{
  ...
  id=((id3<<8)&0xff00)|id4; // 0x6814,16bit ID
...
LCM_DRIVER rm68140_jinping_JP35H038A0_longteng_mcu18_hvga_lcm_drv =
    {
        .name			= "rm68140_jinping_JP35H038A0_longteng_mcu18_hvga",
