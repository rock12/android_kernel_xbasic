...
static void init_lcm_registers(void)
{
    ...
    printf("MYCAT LCD r61408_nisin_NST397WV3938ANP_V1_LG_mcu18_wvga\n");
		...
		send_data_cmd(0x0003);  //0x03    1031
		send_data_cmd(0x0001);  //0x01    1031
		send_data_cmd(0x0001);
		send_data_cmd(0x0003);
		send_data_cmd(0x0000);
		
		send_ctrl_cmd(0x00C4); //add 1031
		send_data_cmd(0x0000); 
		send_data_cmd(0x0001); 
		
		send_ctrl_cmd(0x00C6); //add 1031
		send_data_cmd(0x0000); 
		send_data_cmd(0x0000); 
		
		send_ctrl_cmd(0x00C7); //add 1031
		send_data_cmd(0x0011); 
		send_data_cmd(0x008d); 
		send_data_cmd(0x00a0); 
		send_data_cmd(0x00f5); 
		send_data_cmd(0x0027); 
...
send_data_cmd(0x0028); //RTN
send_data_cmd(0x000c); //BPX16 VBPE xx BP[4:0]
send_data_cmd(0x000a); //EFX16 xx xx FP[4:0]
send_data_cmd(0x0000); //ACBF1[1:0] ACF[1:0] ACBR[1:0] ACR[1:0]
send_data_cmd(0x0000); //ACBF2[1:0] ACF2[1:0] ACBR2[1:0] ACR2[1:0]
send_data_cmd(0x0000); //ACCYC[1:0] VGSET ACFIX[1:0]
send_data_cmd(0x0021);
...
send_ctrl_cmd(0x00C2);
send_data_cmd(0x0010); //LINEINV[2:0] PNSET
...
send_data_cmd(0x0000);  //0x00 内部升压  0x0c 外部升压
...
send_ctrl_cmd(0x0036);
send_data_cmd(0x0000);         //  36H 0x41 rotate 180

send_ctrl_cmd(0x0035);
send_data_cmd(0x0000);

send_ctrl_cmd(0x003A);
send_data_cmd(0x0066);//55
...
static unsigned int lcm_esd_check(void)
{
  ...
  buffer[0] = read_data_cmd();
	...
	printk("mycat$$ rixiin ###*** esd check buffer[0] = 0x%x\r\n",buffer[0]);
...
// ---------------------------------------------------------------------------
LCM_DRIVER r61408_rixin_NST397WV3939AND_mcu18_LG_wvga_lcm_drv = 
{
    .name			= "r61408_rixin_NST397WV3939AND_mcu18_LG_wvga",
