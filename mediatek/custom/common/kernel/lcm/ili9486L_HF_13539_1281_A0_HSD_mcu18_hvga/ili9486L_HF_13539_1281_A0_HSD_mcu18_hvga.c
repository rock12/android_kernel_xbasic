...
{
    unsigned char i,n;
    Lcd_Log("mycat LCD ili9488_HF_140391260_A0_IVO_mcu18_hvga\n");    
    ...
    send_ctrl_cmd(0xF1);
		...
		send_ctrl_cmd(0xC0);
		send_data_cmd(0x23);//VREG1OUT POSITIVE
		send_data_cmd(0x1a);//VREG2OUT NEGATIVE
		
		send_ctrl_cmd(0xC1);
		send_data_cmd(0x44);//VGH,VGL    VGH>=14V.
		send_data_cmd(0x00);
		...
		send_ctrl_cmd(0xC5);
		send_data_cmd(0x00);
		send_data_cmd(0x28);//VCM_REG[7:0]. <=0X80.
		
		send_ctrl_cmd(0xB1);
		send_data_cmd(0xB0);//0XB0 =70HZ, <=0XB0.
		send_data_cmd(0x11);
		
		send_ctrl_cmd(0xB4);
		send_data_cmd(0x02); //2 DOT FRAME MODE,F<=70HZ.
		
		send_ctrl_cmd(0xB6);
		send_data_cmd(0x00);
		send_data_cmd(0x22);//0 GS SS SM ISC[3:0];//42
		send_data_cmd(0x3B);
...
//    sw_clear_panel(0x0);
    MDELAY(10);
#else
   // lcm_compare_id(); // by liuxd, for test ...
    lcm_init();
#endif
}
...
LCM_DRIVER ili9486L_HF_13539_1281_A0_HSD_mcu18_hvga_lcm_drv =
    {
        .name            = "ili9486L_HF_13539_1281_A0_HSD_mcu18_hvga",
