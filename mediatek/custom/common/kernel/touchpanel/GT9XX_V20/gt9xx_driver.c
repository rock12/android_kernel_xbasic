...
#ifdef TPD_HAVE_BUTTON
    u8 keys[TPD_KEY_COUNT] = {0};
    static u8 key_old[TPD_KEY_COUNT] = {0};
#endif
	  ...
	  key_old[i] = keys[i];
        ...
        if (touch_num)
        {
            for (i = 0; i < touch_num; i++)
            {
	 		...
	 		{
				 input_x =GTP_MAX_WIDTH - TPD_WARP_X(abs_x_max, input_x);
				 input_y = GTP_MAX_HEIGHT - TPD_WARP_Y(abs_y_max, input_y);
	 	    }
