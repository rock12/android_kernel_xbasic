...
#endif
#define TP_WAKE_PARAFILE "/data/tp_wake_data"
...
/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
static struct wake_lock touchpanel_wake_lock;
/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
#endif
...
static ssize_t show_gesture(struct device *dev, struct device_attribute *attr, char *buf)
{
    int len = -1;
    if ( buf != NULL ){
        len = snprintf( buf, PAGE_SIZE, "%c", gesture );
...
static DEVICE_ATTR(gesture, 0777, show_gesture, store_gesture);
/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
...
reset_proc:
   ...
   if(TPD_POWER_SOURCE_CUSTOM == MT6323_POWER_LDO_VGP1)
   	{
...
#endif
#if GTP_GESTURE_WAKEUP
    input_set_capability(tpd->dev, EV_KEY, KEY_TP_WAKE);
	/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
...
/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
    ...
    if(device_create_file(firmware_cmd_dev, &dev_attr_gesture) < 0) // /sys/class/mtk-tpd/device/gesture
        {
...
int write_tp_wake_data2file(void *buffer, int len){
    struct file *fp;
    int ret = -1;
    ...
    fp = filp_open(TP_WAKE_PARAFILE,O_WRONLY|O_CREAT, 0644);
    if ( !IS_ERR(fp) ){
        set_fs(KERNEL_DS);
    fp->f_pos = 0;
        ...
        /* input_report_key(tpd->dev, KEY_POWER, 1);
            ...
            {               
                    ...
                    )
                {
                    ...
                    write_tp_wake_data2file((void*)"C",1);
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, KEY_TP_WAKE, 0);
                    input_sync(tpd->dev);
                    // clear 0x814B
                    doze_buf[2] = 0x00;
                ...
                if ( doze_buf[2] == 'c' )
                {
                    ...
                    //write_tp_wake_data2file((void*)"c",1);
                    gesture = 'c';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, KEY_TP_WAKE, 0);
                ...
                else if (doze_buf[2] == 'm') 
                {
                    ...
                    doze_status = DOZE_WAKEUP;
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
                    //write_tp_wake_data2file((void*)"m",1);
                    gesture = 'm';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                ...
                else if  (doze_buf[2] == 'e') 
                {
                    ...
                    doze_status = DOZE_WAKEUP;
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
                    //write_tp_wake_data2file((void*)"e",1);
                    gesture = 'e';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                ...
                else if (doze_buf[2] == 'o')
                {
                    ...
                    doze_status = DOZE_WAKEUP;
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
                    //write_tp_wake_data2file((void*)"o",1);
                    gesture = 'o';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                ...
                else if (doze_buf[2] == 'w') 
                {
                    ...
                    gesture = 'w';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
                    //write_tp_wake_data2file((void*)"w",1);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, KEY_TP_WAKE, 0);
                ...
                else if (doze_buf[2] == 0xAA)//Right
                {
                    ...
                    doze_status = DOZE_WAKEUP;
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
                    //write_tp_wake_data2file((void*)"S",1);
                    gesture = 'S';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                ...
                else if (doze_buf[2] == 0xBA)//up
                {
                    ...
                    doze_status = DOZE_WAKEUP;
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin         */
                    //write_tp_wake_data2file((void*)"U",1);
                    gesture = 'U';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    input_report_key(tpd->dev, KEY_TP_WAKE, 1);
                ...
                else if (0xCC == doze_buf[2])//双击
                {
                    GTP_INFO("Double click to light up the screen!");
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  begin        */
                    //write_tp_wake_data2file((void*)"d",1);
                    gesture = 'd';
                    wake_lock_timeout(&touchpanel_wake_lock, 10 * HZ);
					/*   add by xialeijie for bug when insert sd card cpu can't wake up 20140523  end         */
                    doze_status = DOZE_WAKEUP;
