...
u8 cfg_len = 0;
#if defined(BIRD_TP_PARAM_13C43_JFC43_2)
u8 g_sensor_id = 0;
#endif
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    #if defined(BIRD_TP_PARAM_13C43_JFC43_2)
    g_sensor_id = rd_cfg_buf[GTP_ADDR_LENGTH];
    #endif
...
#if GTP_CUSTOM_CFG
#if defined(BIRD_TP_PARAM_13C43_JFC43_2)
if(rd_cfg_buf[GTP_ADDR_LENGTH]==1)
	{
#endif
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 2)
    {
        ...
        GTP_ERROR("mycat GTP_REG_HW_INFO : %08X", hw_info);
        GTP_ERROR("mycat GTP i2c test failed time %d.", retry);
        msleep(10);
    }
...
reset_proc:
	printk("MYCAT tpd_power_on\n");

    //power on, need confirm with SA
    hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_2800, "TP");
    mdelay(400);
   //printk("MYCAT tpd_power_on 001\n");


    gtp_reset_guitar(client, 20);
	//printk("MYCAT tpd_power_on 002\n");

    ret = gtp_i2c_test(client);
	//printk("MYCAT tpd_power_on 003\n");
    ...
    do
    {
        ...
        if (touch_num)
        {
            for (i = 0; i < touch_num; i++)
            {
				...
				#if defined(BIRD_TP_PARAM_13C43_JFC43_2)
				if (g_sensor_id ==1 ){
                if ( input_y < 820 && input_y >790 ) input_y = 796;
                else if( input_y < 840 && input_y >819 ) 
                	 goto exit_work_func;
        }
				#endif
