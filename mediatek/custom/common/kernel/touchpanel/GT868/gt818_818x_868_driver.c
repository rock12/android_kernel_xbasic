...
reset_proc:
    ...
    {
        err = i2c_read_bytes(client, TPD_SENSOR_ID_REG, &cfg_sensor_id, 1);
    ...
    if (chip_type == GT818)
    {
        ...
        cfg_data_with_charger = gt818_cfg_data_with_charger_ptr[cfg_sensor_id];
    }
    else if(chip_type == GT818X)
    {
        ...
        cfg_data_with_charger = gt818x_cfg_data_with_charger_ptr[cfg_sensor_id];
    }
    else
    {
        ...
        cfg_data_with_charger = gt868_cfg_data_with_charger_ptr[cfg_sensor_id];
        gt868_div_2 = 0;
        if(0x0f == cfg_data[51])
        {
            gt868_div_2 = 1;
...
static void force_reset_guitar(void)
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        //Send config
        ret = i2c_write_bytes(i2c_client, TPD_CONFIG_REG_BASE, cfg_data, CONFIG_LEN);
...
#if (defined(MT6575) || defined(MT6577)|| defined(MT6572))
    if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT == get_boot_mode())
    {
        tpd_button(x, y, 0);
    }
